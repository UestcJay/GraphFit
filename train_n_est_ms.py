...
def parse_arguments():
    ...
    parser.add_argument('--name', type=str, default='GraphFit', help='training run name')
    ...
    parser.add_argument('--refine_epoch', type=int, default=540, help='refine model from this epoch')
...
def train_pcpnet(opt):
    ...
    if opt.scheduler_type == 'step':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[200, 500], gamma=0.1) # milestones in number of optimizer iterations
    else:
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                loss, reg_trans, reg_weights, err_angle, consistency_loss, normal_loss = compute_loss(
                    pred=pred, target=target,
                    outputs=opt.outputs,
                    output_pred_ind=output_pred_ind,
                    output_target_ind=output_target_ind,
    ...
    for oi, o in enumerate(outputs):
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if use_consistency:
                ...
                loss = consistency_loss1 + normal_loss
        else:
            raise ValueError('Unsupported output type: %s' % (o))
...
def get_model(opt, log_dirname):
    ...
    os.system('cp %s %s' % (
        './models/GraphFit_ms.py', os.path.join(log_dirname, 'DeepFit.py')))  # backup the current model
    return model
